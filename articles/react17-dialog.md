---
title: "【React】コンポーネントの外側をクリックしたら閉じるダイアログを実装する"
emoji: "💬"
type: "tech"
...
題名のようなコンポーネントを自作する機会があり、調査と実装の過程をまとめておきます。
使う技術は以下の通りです。
- react 16.14.0
- reat-dom 16.14.0
- typescript
...
# 余談： React17におけるevent delegationの破壊的変更
ここからは余談です。
React17でイベントの委譲(event delegation)に破壊的変更がありました。
ただ、今回紹介したコードはReact17でも問題なく動作するので安心してください。

本記事実装したようなDialogは問題になりませんが、似たようなコンポーネントでもコードの書き方によっては修正が必要な場合もあります。
気になる方は以下の記事にまとめたので見てみてください。
